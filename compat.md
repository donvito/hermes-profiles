...
| `config.yaml → skills.defaults` | CLI / TUI / gateway / cron session startup (`agent/skill_commands.py`) | Profile-scoped skills preloaded on every new session, same as `hermes -s …`. Missing names warn and are skipped. Harmless on older Hermes that ignore the key. |
